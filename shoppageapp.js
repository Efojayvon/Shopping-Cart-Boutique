...
displayProducts(products){
    ...
    products.forEach(product=>{
        result+=`        <!--dummy product-->
        <article class="product">
            <div class="img-container">
                <img src=${product.image}
            </div>
            <h3>${product.title} </h3>
            <h4>${product.price} GHS</h4>
            <h3><button class="bag-btn" data-id=${product.id}><i class="fas fa-shopping-cart"></i>
                add to cart
            </button></h3>
...
setCartValues(cart){
 ...
 addCartItem(item){
   ...
   hideCart(){cartOverlay.classList.remove('transparentBcg');
...
while(cartContent.children.length > 0){
    cartContent.removeChild(cartContent.chlidren[0]);
}
this.hideCart();
}
